...
def ternary_tensor(Time, aO, aH, aL, aC, aV, bO, bH, bL, bC, bV, aR, bR, cC, dC):
    count = 0
    X, Y = [], []
    for i in range(0, len(Time) - WINDOW - FORECAST, STEP):
        ...
        cc = (np.array(cc) - cc_mean) / cc_std
        dc = (np.array(dc) - dc_mean) / dc_std
        ...
        x_i = np.column_stack((ao, ah, al, ac, av, bo, bh, bl, bc, bv, ar, br, cc, dc, s))
...
def format_data(df, ternary=1, binary=0):
    ...
    aR = df.Volume_y.tolist()
    bR = df.Volume_y.tolist()

    cC = df.Close_z.tolist()
    dC = df.Close_a.tolist()

    if (ternary == 1):
        return ternary_tensor(Time, aO, aH, aL, aC, aV, bO, bH, bL, bC, bV, aR, bR, cC, dC)

    elif (binary == 1):
        return binary_tensor(Time, aO, aH, aL, aC, aV, bO, bH, bL, bC, bV, aR, bR, cC, dC)
...
cc_mean = np.mean(df2['Close_z'])
dc_mean = np.mean(df2['Close_a'])
cc_std = np.std(df2['Close_z'])
dc_std = np.std(df2['Close_a'])
...
if TRAINING:
    for x in xrange(len(history.history['acc'])):
